...
def _attn_fwd_inner_full_int8(acc, l_i, m_i, q,  #
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        p = (tl.math.exp2(qk)*127).to(tl.int8)
        l_ij = tl.sum(p, 1)
        # -- update m_i and l_i
        ...
        acc = acc + tl.dot(p, v, out_dtype=tl.int32).to(tl.float32)*v_scale
        # update m_i and l_i
        m_i = m_ij
...
@triton.jit
def _attn_fwd_full_int8(Q, K, V, Q_scale, K_scale, V_scale, sm_scale, M, Out,  #
    ...
    if STAGE & 1:
        acc, l_i, m_i = _attn_fwd_inner_full_int8(acc, l_i, m_i, q, K_block_ptr, V_block_ptr, q_scale, K_block_scale_ptr, v_scale, #
                                        start_m, qk_scale,  #
...
class _attention_full_int8(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, q_scale, k_scale, v_scale, causal, sm_scale):
        ...
        _attn_fwd_full_int8[grid](
            q, k, v, q_scale, k_scale, v_scale, sm_scale, M, o,  #
